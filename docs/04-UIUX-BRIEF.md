...
- **`Login.tsx`**: content row wraps on `isCompact`. The user-locked desktop logo (`height:140, marginTop:-90` — "do not change again") retains those exact desktop values. On phones, the welcome words and the ZOTO logo sit together on one centered horizontal line, followed immediately by the divider/tagline and then the centered login card — matching the compact single-column reference layout without changing ZOTO's white brand theme. Login card: `flex:"1 1 auto", width:"100%", maxWidth:460` on compact — centered, not left-anchored. Footer stacks copyright/links vertically on `isMobile`.
