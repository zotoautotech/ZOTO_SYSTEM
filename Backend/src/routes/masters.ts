...
const CUSTOMER_LIST_FIELDS = [
  ...
  // Misspelled in the live sheet (see CLAUDE.md) — this is the doer a customer is assigned
  // to, used by the punch form to warn when someone picks a customer that isn't theirs.
  "Field Sale Repersentative",
];
...
mastersRouter.get("/customers", async (req, res, next) => {
  try {
    ...
    // Every doer sees every customer here (deliberate — the picker is not access-gated);
    // the assigned-rep field rides along so the punch form can tell the doer who owns a
    // customer they aren't assigned to. Punching itself is what's actually restricted,
    // enforced server-side in POST /orders.
    const slim = rows
